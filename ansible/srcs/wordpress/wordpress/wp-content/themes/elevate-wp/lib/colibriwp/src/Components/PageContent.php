...
class PageContent extends ComponentBase {
	...
	public function wrapPostContent( $content ) {
		...
		wp_link_pages(
			array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'elevate-wp' ) . '</span>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'pagelink'    => '<span class="screen-reader-text">' . __( 'Page', 'elevate-wp' ) . ' </span>%',
				'separator'   => '<span class="screen-reader-text">, </span>',
			)
		);

			$link_pages = ob_get_clean();
		return "<div class='kubio-post-content'>{$content} {$link_pages}</div>";
	}
