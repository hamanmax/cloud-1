...
class Flags {
	...
	private function withFlags( $action, $flag = null, $data = null ) {
		...
		if ( $action === 'get' ) {
			...
			$elevate_wp_flags_defaults = apply_filters( 'kubio/instance-flags-default', array() );

			if ( isset( $elevate_wp_flags_defaults[ $flag ] ) ) {
				return $elevate_wp_flags_defaults[ $flag ];
			}
