...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'elevate-wp' ),
				'menu_title'                      => __( 'Install Plugins', 'elevate-wp' ),
				/* translators: %s: plugin name. */
				'installing'                      => __( 'Installing Plugin: %s', 'elevate-wp' ),
				/* translators: %s: plugin name. */
				'updating'                        => __( 'Updating Plugin: %s', 'elevate-wp' ),
				'oops'                            => __( 'Something went wrong with the plugin API.', 'elevate-wp' ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', 'elevate-wp' ),
				'dashboard'                       => __( 'Return to the Dashboard', 'elevate-wp' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'elevate-wp' ),
				...
				'dismiss'                         => __( 'Dismiss this notice', 'elevate-wp' ),
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'elevate-wp' ),
				esc_html__( 'Update Required', 'elevate-wp' )
			);
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$rendered .= sprintf(
							$line_template,
							sprintf(
								translate_nooped_plural( $this->strings[ $type ], $count, 'elevate-wp' ),
								$imploded,
								$count
							)
						);
		...
		protected function create_user_action_links_for_notice(
			...
			if ( current_user_can( 'install_plugins' ) ) {
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['install_link'], $install_count, 'elevate-wp' ),
				...
				if ( $update_count > 0 ) {
					$action_links['update'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['update_link'], $update_count, 'elevate-wp' ),
						esc_url( $this->get_tgmpa_status_url( 'update' ) )
					);
				}
			}
			...
			if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					$link_template,
					translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'elevate-wp' ),
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return __( 'Required', 'elevate-wp' );
			}

			return __( 'Recommended', 'elevate-wp' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = __( 'WordPress Repository', 'elevate-wp' );
					break;
				case 'external':
					$string = __( 'External Source', 'elevate-wp' );
					break;
				case 'bundled':
					$string = __( 'Pre-Packaged', 'elevate-wp' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return __( 'Not Installed', 'elevate-wp' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = __( 'Installed But Not Activated', 'elevate-wp' );
			} else {
				$install_status = __( 'Active', 'elevate-wp' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Required Update not Available', 'elevate-wp' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = __( 'Requires Update', 'elevate-wp' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Update recommended', 'elevate-wp' );
			}
			...
			/* translators: 1: install status, 2: update status */
				_x( '%1$s, %2$s', 'Install/Update Status', 'elevate-wp' ),
				$install_status,
				$update_status
			);
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'elevate-wp' ),
				'source' => __( 'Source', 'elevate-wp' ),
				'type'   => __( 'Type', 'elevate-wp' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = __( 'Version', 'elevate-wp' );
				$columns['status']  = __( 'Status', 'elevate-wp' );
			}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = __( 'Install %2$s', 'elevate-wp' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = __( 'Update %2$s', 'elevate-wp' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['activate'] = __( 'Activate %2$s', 'elevate-wp' );
				}
			}
		...
		public function wp_plugin_update_row( $slug, $item ) {
			...
			esc_html__( 'Upgrade message from the plugin author:', 'elevate-wp' ),
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = __( 'Install', 'elevate-wp' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = __( 'Update', 'elevate-wp' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = __( 'Activate', 'elevate-wp' );
				}
			}
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins were selected to be installed. No action taken.', 'elevate-wp' );
					} else {
						$message = __( 'No plugins were selected to be updated. No action taken.', 'elevate-wp' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins are available to be installed at this time.', 'elevate-wp' );
					} else {
						$message = __( 'No plugins are available to be updated at this time.', 'elevate-wp' );
					}
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'elevate-wp' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'elevate-wp' );
					}
